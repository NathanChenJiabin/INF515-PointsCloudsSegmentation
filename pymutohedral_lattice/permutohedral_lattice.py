...
class HashTablePermutohedral(object):
    def __init__(self, kd_, vd_):
        """
        :param kd_: int, the dimensionality of the position vectors on the hyperplane.
        :param vd_: int, the dimensionality of the value vectors
        """
        self.kd = kd_
        self.vd = vd_
        ...
        self.keys = np.zeros((int(kd_ * self.capacity / 2)), dtype='int16')
        self.values = np.zeros((int(vd_ * self.capacity / 2)), dtype='float32')
    ...
    def lookup(self, k, create=True):
        ...
        :param k:  numpy array, a list of keys to match
        :param create: bool, whether a new entry should be created it nothing is matched
        :return: The index of the first object in k, when k is completely matched
        """
    ...
    def _grow(self):
        ...
        new_values = np.zeros((int(self.vd * self.capacity / 2)), dtype='float32')
        new_values[:int(self.vd * old_capacity / 2)] = self.values
        self.values = new_values

        # Migrate the key vectors.
        new_keys = np.zeros((int(self.kd * self.capacity / 2)), dtype='int16')
        new_keys[:int(self.kd * old_capacity / 2)] = self.keys
        self.keys = new_keys
...
class PermutohedralLattice(object):
    ...
    def __init__(self, d, vd, inp_len):
        ...
        :param d: int, dimensionality of key vectors
        :param vd: int, dimensionality of value vectors
        :param inp_len: int, number of points in the input
        """
    ...
    def filter(inp, ref, debug=True):
        """
        :param inp: numpy array, the image to filter, should have the shape (rows, cols, channels)
        :param ref: numpy array, the lattice to use for filtering
        :param debug: boolean, default True, whether the function should print its current state and the different run times
        :return: numpy array, output image after filtered
        """
    ...
    def splat(self, position, pos_idx, value):
        ...
        somme = np.sum(self.greedy) / self.d1
        ...
        if somme > 0:
            ...
            cond_mask = self.rank >= self.d1 - somme
            ...
            cond_mask = self.rank < -somme
            self.greedy[cond_mask] += self.d1
            self.rank[cond_mask] += self.d1

        # self.rank += sum
        self.rank = np.add(self.rank, somme, out=self.rank, casting="unsafe")
